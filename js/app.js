...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // If running on static host / GitHub Pages, pre-warm Pyodide in background
  const isStaticHost = window.location.hostname.endsWith('github.io') || window.location.protocol === 'file:' || !window.location.port;
  if (isStaticHost) {
    setTimeout(() => {
      getPyodideEngine().catch(e => console.log('Background WASM pre-warm:', e));
    }, 500);
  }
});
...
// In-Browser Python WASM Runtime Manager (Pyodide)
...
async function getPyodideEngine() {
  ...
  pyodideInitPromise = (async () => {
    const stateEl = document.getElementById('status-pipeline-state');
    if (stateEl) {
      stateEl.textContent = 'Initializing in-browser Python WASM engine...';
      stateEl.style.color = '#38bdf8';
    }

    if (typeof loadPyodide === 'undefined') {
      throw new Error('Pyodide WebAssembly runtime script is not loaded. Please verify internet connection.');
    }
    ...
    if (stateEl) {
      stateEl.textContent = 'Mounting EDA packages...';
    }

    // Mount EDA python source files into Pyodide virtual filesystem
    if (window.EDA_PYTHON_FILES) {
      for (const [filePath, content] of Object.entries(window.EDA_PYTHON_FILES)) {
    ...
    // Prepare EDAPipeline Python entrypoint
...
def run_eda_pipeline_json(params_str):
...
`);
    ...
    if (stateEl) {
      stateEl.textContent = 'Ready (In-Browser WASM Engine)';
...
async function loadExamples() {
  ...
  // 1. Synchronous bundled data check
  if (window.EDA_EXAMPLES && Object.keys(window.EDA_EXAMPLES).length > 0) {
    examples = window.EDA_EXAMPLES;
  } else {
    // 2. Fetch from local server or static examples.json
    try {
      ...
      try {
        const res2 = await fetch('./examples.json');
        if (res2.ok) {
          examples = await res2.json();
        }
      } catch (e2) {}
    }
  }
  ...
  if (!examples) {
    console.warn('Could not load examples from any source');
    return;
  }
  ...
  try {
    ...
    console.warn('Could not populate examples:', e);
  }
}
...
async function runAnalysis() {
  ...
  try {
    ...
    // If running on local server, prioritize native python server
    ...
    if (isLocalServer) {
      try {
        ...
        console.log('Local server API unreachable, falling back to in-browser WASM...', err);
      }
    }

    // Client-side Pyodide WASM execution (for GitHub Pages & static hosting)
    if (!data) {
      const stateEl = document.getElementById('status-pipeline-state');
      if (stateEl) {
        stateEl.textContent = 'Running in-browser Python WASM...';
        stateEl.style.color = '#38bdf8';
      }
      const pyodide = await getPyodideEngine();
      const runEda = pyodide.globals.get('run_eda_pipeline_json');
      const resultJson = runEda(JSON.stringify(payload));
      data = JSON.parse(resultJson);
      execMode = 'Client WASM';
    }
    ...
    const stateEl = document.getElementById('status-pipeline-state');
    if (stateEl) {
      stateEl.textContent = `Completed (${execMode})`;
      stateEl.style.color = '#10b981';
    }
  } catch (err) {
